...
class PermissionService
{
     ...
     * @throws Exception
     */
    public function allPermission(User $user, Role $role)
    {
        try {
            $permissions     = Permission::get();
            $userPermissions = Permission::join(
            ...
            $allPermissions = $allPermissions->merge($userPermissions);
            $allPermissions = $allPermissions->pluck('name', 'id');
            return AppLibrary::permissionWithAccess($permissions, $allPermissions);
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception(QueryExceptionLibrary::message($exception), 422);
        }
