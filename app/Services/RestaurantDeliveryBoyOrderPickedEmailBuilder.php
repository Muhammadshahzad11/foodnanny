...
use App\Mail\RestaurantDeliveryBoyPickedOrder;
...
class RestaurantDeliveryBoyOrderPickedEmailBuilder
{
    public int $orderId;
    public object $order;
            ...
            if (count($emailArray) > 0) {
                try {
                    ...
                    if ($notificationAlert && $notificationAlert->mail == SwitchBox::ON) {
                        try {
                            Mail::to($emailArray[0])->cc($emailArray)->send(new RestaurantDeliveryBoyPickedOrder($this->order->order_serial_no, $notificationAlert->mail_message));
                        } catch (Exception $e) {
