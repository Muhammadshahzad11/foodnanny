...
class RoleService
{
     ...
     * @throws Exception
     */
    public function store(RoleRequest $request): \Illuminate\Database\Eloquent\Model|\Illuminate\Database\Eloquent\Builder
    {
        try {
    ...
    public function destroy(Role $role): void
    {
        try {
            if (!in_array($role->id, $this->roleArray)) {
                ...
                throw new Exception(trans('all.role_not_deletable'), 422);
            }
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception(QueryExceptionLibrary::message($exception), 422);
