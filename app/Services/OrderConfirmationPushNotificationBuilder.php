...
class OrderConfirmationPushNotificationBuilder
{
    public int $orderId;
    public mixed $status;
    public object $order;
    ...
    private function notification($fcmTokenArray, $orderId, $message, $orderStatus, $url = null): void
    {
        try {
            ...
            $firebase->sendNotification($pushNotification, $fcmTokenArray, "regular-order", $orderStatus, $url);
        } catch (Exception $e) {
    ...
    private function pending($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::PENDING);
        }
    }
    ...
    private function confirmation($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::ACCEPT, "/my-orders/{$orderId}");
        }
    }
    ...
    private function preparing($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::PREPARING, "/my-orders/{$orderId}");
        }
    }
    ...
    private function prepared($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::PREPARED, "/my-orders/{$orderId}");
        }
    }
    ...
    private function outForDelivery($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::OUT_FOR_DELIVERY, "/my-orders/{$orderId}");
        }
    }
    ...
    private function delivered($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::DELIVERED, "/my-orders/{$orderId}");
        }
    }
    ...
    private function canceled($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::CANCELED);
        }
    }
    ...
    private function rejected($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::REJECTED, "/my-orders/{$orderId}");
        }
    }
    ...
    private function returned($fcmTokenArray, $orderId): void
    {
        ...
        if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
            $this->notification($fcmTokenArray, $orderId, $notificationAlert->push_notification_message, (string) OrderStatus::RETURNED, "/my-orders/{$orderId}");
        }
    }
}
