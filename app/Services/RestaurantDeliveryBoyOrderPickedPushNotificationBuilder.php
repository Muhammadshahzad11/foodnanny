...
class RestaurantDeliveryBoyOrderPickedPushNotificationBuilder
{
    public int $orderId;
    public object $order;
                ...
                try {
                    ...
                    if ($notificationAlert && $notificationAlert->push_notification == SwitchBox::ON) {
                        $pushNotification = (object)[
                            'title'       => 'Delivery boy order pick confirmation',
                            'description' => $notificationAlert->push_notification_message,
                            'order_id'    => $this->orderId
                        ];
                        $firebase         = new FirebaseService();
                        $firebase->sendNotification($pushNotification, $fcmTokenArrays, 'delivery-boy-order', 'picked', "/admin/online-orders/show/{$this->orderId}");
                    }
                } catch (Exception $e) {
