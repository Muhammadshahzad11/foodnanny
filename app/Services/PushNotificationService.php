...
class PushNotificationService
{
    ...
    public function store(PushNotificationRequest $request): PushNotification
    {
        try {
            ...
            if ($pushNotification->role_id == 0 && $pushNotification->user_id == 0) {
                ...
                if ($pushNotification->role_id !== 0 && $pushNotification->user_id == 0) {
                    $fcmWebDeviceToken    = User::role($pushNotification->role_id)->whereNotNull('web_token')->pluck('web_token')->toArray();
                    $fcmMobileDeviceToken = User::role($pushNotification->role_id)->whereNotNull('device_token')->pluck('device_token')->toArray();
                } else {
                    $fcmWebDeviceToken    = User::where(['id' => $pushNotification->user_id])->whereNotNull('web_token')->pluck('web_token')->toArray();
                    $fcmMobileDeviceToken = User::where(['id' => $pushNotification->user_id])->whereNotNull('device_token')->pluck('device_token')->toArray();
                }
            }
     ...
     * @throws Exception
     */
    public function destroy(PushNotification $pushNotification): void
    {
        try {
