...
class RoleController extends AdminController implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('permission:system_settings|push-notifications|push-notifications_create|employees|credit-balance-report', only: ['index']),
            new Middleware('permission:system_settings', only: ['show']),
            new Middleware('permission:system_settings', only: ['store']),
            new Middleware('permission:system_settings', only: ['update']),
            new Middleware('permission:system_settings', only: ['destroy']),
        ];
    }
