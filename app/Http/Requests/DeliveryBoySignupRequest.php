...
class DeliveryBoySignupRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'phone'                => ['required', 'string', Rule::unique("users", "phone")->whereNull('deleted_at')->where('is_guest', Ask::NO)],
    ...
    public function withValidator($validator): void
    {
        if ($this->required) {
            $validator->after(function ($validator) {
                $otp = DB::table('one_time_passwords')->where('code', request('country_code'))->where('provider', request('phone'))->where('token', request('token'));
                if ($otp->exists()) {
