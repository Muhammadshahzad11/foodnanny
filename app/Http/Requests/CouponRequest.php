...
class CouponRequest extends FormRequest
{
    ...
    {
        return [
            'name'             => ['required', 'string', 'max:190', Rule::unique("coupons", "name")->ignore($this->route('coupon.id'))->where('restaurant_id', $this->restaurant())],
            'description'      => ['nullable', 'string', 'max:900'],
            'code'             => ['required', 'string', 'max:24', Rule::unique("coupons", "code")->ignore($this->route('coupon.id'))->where('restaurant_id', $this->restaurant())],
            'discount'         => ['required', 'numeric'],
            'discount_type'    => ['required', 'numeric', 'max:24'],
            'start_date'       => ['required', 'string'],
            'end_date'         => ['required', 'string'],
            'minimum_order'    => ['required', 'numeric'],
            'maximum_discount' => request('discount_type') === DiscountType::PERCENTAGE ? ['required', 'numeric'] : ['nullable', 'numeric'],
