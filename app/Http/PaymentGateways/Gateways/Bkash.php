...
class Bkash extends PaymentAbstract
{
    ...
    public function payment($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            Config::set('bkash.callbackURL', route('payment.success', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'bkash']));

            $request['intent']                = 'sale';
            $request['mode']                  = '0011';
            $request['payerReference']        = $frontendOrder->order_serial_no;
            $request['currency']              = $currencyCode;
            $request['amount']                = (float)number_format($frontendOrder->total);
            $request['merchantInvoiceNumber'] = $frontendOrder->order_serial_no;
            $request['callbackURL']           = route('payment.success', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'bkash']);
            ...
            if (isset($bkash['bkashURL'])) {
                ...
                return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'bkash'])->with(
                    'error',
                    $bkash['statusMessage']
                );
            }
    ...
    public function success($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            if ($request['status'] === "success" && $request['paymentID']) {
                $this->paymentService->payment($frontendOrder, 'bkash', $request['paymentID']);
                return redirect()->route('payment.successful', ['frontendOrder' => $frontendOrder])->with(
                    'success',
                    trans('all.message.payment_successful')
                );
            } else {
                return redirect()->route('payment.fail', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'bkash'])->with(
                    'error',
    ...
    {
        return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'bkash'])->with('error', trans('all.message.something_wrong'));
    }
