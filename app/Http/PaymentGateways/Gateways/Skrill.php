...
class Skrill extends PaymentAbstract
{
    ...
    public function payment($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            $this->skrill->return_url     = route('payment.successful', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'skrill']);
            $this->skrill->cancel_url     = route('payment.cancel', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'skrill']);
            $this->skrill->status_url     = route('payment.success', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'skrill']);
            $this->skrill->prepare_only   = 1;
            $this->skrill->amount         = $frontendOrder->total;
            $this->skrill->currency       = $currencyCode;
            $this->skrill->language       = 'EN';
            $this->skrill->invoice_id     = $frontendOrder->order_serial_no;
            $this->skrill->customer_email = $frontendOrder->user?->email;
    ...
    public function success($frontendOrder, $request): void
    {
        try {
            if (isset($request['transaction_id'])) {
                $this->paymentService->payment($frontendOrder, 'skrill', $request['transaction_id']);
            } else {
    ...
    {
        return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'skrill'])->with('error', trans('all.message.something_wrong'));
    }
