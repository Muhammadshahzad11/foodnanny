...
class Payfast extends PaymentAbstract
{
    ...
    public function payment($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            $data       = [
                ...
                'return_url'    => route('payment.success', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'payfast']),
                'cancel_url'    => route('payment.cancel', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'payfast']),
                'notify_url'    => route('payment.success', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'payfast']),
                'name_first'    => $frontendOrder->user?->FirstName,
                'name_last'     => $frontendOrder->user?->LastName,
                'email_address' => $frontendOrder->user?->email,
                'm_payment_id'  => $frontendOrder->order_serial_no,
                'amount'        => number_format(sprintf('%.2f', $frontendOrder->total), 2, '.', ''),
                'item_name'     => $frontendOrder->order_serial_no
            ];
            ...
            if (isset($payfastUrl)) {
                ...
                return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'payfast'])->with(
                    'error',
    ...
    public function success($frontendOrder, $request)
    {
        try {
            if (isset($request['m_payment_id']) && $request['status'] == 'COMPLETED') {
                $this->paymentService->payment($frontendOrder, 'payfast', $request['m_payment_id']);
                return redirect()->route('payment.successful', ['frontendOrder' => $frontendOrder])->with(
                    'success',
                    trans('all.message.payment_successful')
                );
            } else {
                return redirect()->route('payment.fail', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'payfast'])->with(
                    'error',
                    trans('all.message.something_wrong')
                );
            }
    ...
    {
        return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'payfast'])->with('error', trans('all.message.something_wrong'));
    }
