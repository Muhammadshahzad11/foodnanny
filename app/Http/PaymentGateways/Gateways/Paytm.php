...
class Paytm extends PaymentAbstract
{
    ...
    public function payment($frontendOrder, $request)
    {
        try {
            $data = PaytmWallet::with('receive');
            $data->prepare([
                'order' => $frontendOrder->order_serial_no,
                'user' => $frontendOrder->user?->id,
                'mobile_number' => $frontendOrder->user?->phone,
                'email' => $frontendOrder->user?->email,
                'amount' => number_format((float)$frontendOrder->total, 2, '.', ''),
                'callback_url' => route('payment.success', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'paytm'])
            ]);
    ...
    public function success($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            if (isset($request['TXNID']) && $request['STATUS'] == 'TXN_SUCCESS') {
                $this->paymentService->payment($frontendOrder, 'paytm', $request['TXNID']);
    ...
    {
        return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'paytm'])->with('error', trans('all.message.something_wrong'));
    }
