...
class Paypal extends PaymentAbstract
{
    ...
    public function payment($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            $response = $this->gateway->createOrder([
                "intent"              => "CAPTURE",
                "application_context" => [
                    "return_url" => route('payment.success', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'paypal']),
                    "cancel_url" => route('payment.cancel', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'paypal']),
                ],
                "purchase_units"      => [
                    0 => [
                        "amount" => [
                            "currency_code" => $currencyCode,
                            "value"         => (float)$frontendOrder->total
                        ]
                    ]
                ]
            ]);
            ...
            if (isset($response['id']) && $response['id'] != null) {
                ...
                return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'paypal'])->with(
                    'error',
    ...
    public function success($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            ...
            if (isset($this->response['status']) && $this->response['status'] == 'COMPLETED') {
                $this->paymentService->payment($frontendOrder, 'paypal', $this->response['id']);
                return redirect()->route('payment.successful', ['frontendOrder' => $frontendOrder])->with('success', trans('all.message.payment_successful'));
            } else {
                return redirect()->route('payment.fail', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'paypal'])->with(
                    'error',
    ...
    {
        return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'paypal'])->with('error', trans('all.message.something_wrong'));
    }
