...
class Senangpay extends PaymentAbstract
{
    ...
    public function payment($frontendOrder, $request): \Illuminate\Routing\Redirector|\Illuminate\Contracts\Foundation\Application|\Illuminate\Http\RedirectResponse
    {
        try {
            ...
            $amount  = number_format((float)$frontendOrder->total, 2, '.', '');
            $hashKey = md5($this->secretKey . $detail . $amount . $frontendOrder->order_serial_no);

            $data = http_build_query([
                'detail'   => $detail,
                'amount'   => $amount,
                'order_id' => $frontendOrder->order_serial_no,
                'hash'     => $hashKey,
                'name'     => $frontendOrder->user?->name,
                'email'    => $frontendOrder->user?->email,
                'phone'    => $frontendOrder->user?->phone,
            ]);
    ...
    public function success($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            if (isset($request['status_id']) && $request['status_id'] == '1') {
                $this->paymentService->payment($frontendOrder, 'senangpay', $request['transaction_id']);
    ...
    {
        return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'senangpay'])->with('error', trans('all.message.something_wrong'));
    }
    ...
    {
        $frontendOrder = Order::where('order_serial_no', $request['order_id'])->first();
        try {
            return $this->success($frontendOrder, $request);
        } catch (Exception $e) {
            Log::info($e->getMessage());
            return redirect()->route('payment.fail', [
                'frontendOrder'  => $frontendOrder,
