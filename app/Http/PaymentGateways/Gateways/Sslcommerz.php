...
class Sslcommerz extends PaymentAbstract
{
    ...
    public function payment($frontendOrder, $request): \Illuminate\Routing\Redirector|\Illuminate\Contracts\Foundation\Application|\Illuminate\Http\RedirectResponse
    {
        try {
            ...
            $array['total_amount']     = $frontendOrder->total;
            $array['currency']         = $currencyCode;
            $array['tran_id']          = "SSLCZ_" . uniqid();
            $array['shipping_method']  = "NO";
            $array['cus_name']         = $frontendOrder->user->name;
            $array['cus_email']        = $frontendOrder->user->email;
            $array['cus_add1']         = $frontendOrder->address;
            $array['cus_city']         = "";
            $array['cus_state']        = "";
            $array['cus_postcode']     = "";
            $array['cus_country']      = "";
            $array['cus_phone']        = $frontendOrder->user->phone;
            $array['product_name']     = $frontendOrder->order_serial_no;
            $array['product_category'] = "Food";
            $array['product_profile']  = "general";
            $array['product_amount']   = $frontendOrder->subtotal;
            $array['discount_amount']  = $frontendOrder->discount;
            $array['convenience_fee']  = $frontendOrder->delivery_fee;
    ...
    public function success($frontendOrder, $request): \Illuminate\Http\RedirectResponse
    {
        try {
            if (isset($request['bank_tran_id'])) {
                $paymentService = new PaymentService;
                $paymentService->payment($frontendOrder, 'sslcommerz', $request['bank_tran_id']);
    ...
    {
        return redirect()->route('payment.index', ['frontendOrder' => $frontendOrder, 'paymentGateway' => 'sslcommerz'])->with('error', trans('all.message.something_wrong'));
    }
