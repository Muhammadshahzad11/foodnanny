<template>
                    ...
                    <tbody class="db-table-body">
                        <tr v-for="lang in languages" :key="lang.id" class="db-table-body-tr">
                            ...
                            <td class="db-table-body-td">
                                <span v-if="translationTitle(lang.code)">{{ textShortener(translationTitle(lang.code), 40) }}</span>
    ...
    <CampaignTranslationModalComponent :props="modalProps" />
</template>
...
import CampaignTranslationModalComponent from "./CampaignTranslationModalComponent.vue";

const KEYS = ["title", "description"];

export default {
    name: "CampaignTranslationComponent",
    components: { LoadingComponent, CampaignTranslationModalComponent },
    setup() {
        const campaignStore = useCampaignStore();
        const languageStore = useLanguageStore();
        return { campaignStore, languageStore };
    },
    data() {
        return {
    ...
    methods: {
        ...
        initForm: function () {
            const existing = this.campaign.translations || [];
            const translations = {};
        ...
        translationTitle: function (code) {
            const t = this.modalProps.form.translations[code];
            return t && this.hasValue(t.title) ? t.title : "";
        },
    },
};
</script>
