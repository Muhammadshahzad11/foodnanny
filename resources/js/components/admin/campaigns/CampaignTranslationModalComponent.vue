<template>
    ...
    <div id="campaignTranslationModal" class="modal">
                ...
                <form @submit.prevent="save">
                    <div class="form-row">
                        ...
                        <template v-if="props.selectedLanguage && props.form.translations[props.selectedLanguage]">
                            <div class="form-col-12">
                                <label class="db-field-title">{{ $t('label.title') }} ({{ props.selectedLanguage }})</label>
                                <p v-if="props.campaign.title" class="text-xs text-paragraph bg-gray-50 rounded-md py-1.5 px-2.5 mb-1.5">
                                    <i class="lab lab-line-language"></i> {{ props.campaign.title }}
                                </p>
                                <input v-model="props.form.translations[props.selectedLanguage].title" :dir="isRtl ? 'rtl' : 'ltr'" type="text" class="db-field-control" />
                            </div>
                            <div class="form-col-12">
                                <label class="db-field-title">{{ $t('label.description') }} ({{ props.selectedLanguage }})</label>
                                <p v-if="props.campaign.description" class="text-xs text-paragraph bg-gray-50 rounded-md py-1.5 px-2.5 mb-1.5" v-html="props.campaign.description"></p>
...
export default {
    name: "CampaignTranslationModalComponent",
    ...
    methods: {
        close: function () {
            useModal().closeModal('campaignTranslationModal');
        },
        save: function () {
            this.loading.isActive = true;
            this.campaignStore.saveTranslations({ id: this.$props.props.id, form: this.$props.props.form }).then(res => {
                this.loading.isActive = false;
                useModal().closeModal('campaignTranslationModal');
                alertService.success(res.data.message);
                this.campaignStore.view(this.$props.props.id);
            }).catch(err => {
                this.loading.isActive = false;
