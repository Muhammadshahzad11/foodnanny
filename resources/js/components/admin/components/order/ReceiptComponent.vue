        ...
        <div class="modal-dialog max-w-[340px] rounded-none" id="print">
            <div class="modal-body">
                <div class="text-center pb-3.5 border-b border-dashed border-gray-400">
                    <h3 class="text-2xl font-bold mb-1">{{ orderRestaurant.name }}</h3>
                    <h4 class="text-sm font-normal">{{ orderRestaurant.address }}</h4>
                    <h5 v-if="orderRestaurant.phone" class="text-sm font-normal">{{ $t('label.phone') }}: {{ orderRestaurant.country_code + orderRestaurant.phone }}</h5>
                </div>
                        ...
                        <tr v-if="orderItems.length > 0" v-for="item in orderItems" :key="item">
                            ...
                            <td class="text-left font-normal align-top py-1">
                                <div class="flex items-center justify-between">
                                    ...
                                    <p class="text-xs leading-5 text-heading">{{ item.total_currency_price }}
                                    </p>
                                </div>
                ...
                <div class="py-2 pl-7">
                    <table class="w-full">
                        <tbody>
                        <tr>
                            <td class="text-xs text-left py-0.5 text-heading">{{ $t('label.subtotal') }}</td>
                            <td class="text-xs text-right py-0.5 text-heading">
                                {{ order.subtotal_currency_price }}
                            </td>
                        </tr>
                        <tr v-if="order.order_type === enums.orderTypeEnum.DELIVERY && order.delivery_fee > 0">
                        ...
                        <tr v-if="order.order_type === enums.orderTypeEnum.DELIVERY && order.rider_tip > 0">
                            <td class="text-xs text-left py-0.5 text-heading">
                                {{ $t('label.rider_tip') }}
                            </td>
                            <td class="text-xs text-right py-0.5 text-heading">
                        ...
                        <tr v-if="order.discount > 0">
                            <td class="text-xs text-left py-0.5 text-heading">{{ $t('label.discount') }}</td>
                            <td class="text-xs text-right py-0.5 text-heading">- {{ order.discount_currency_price }}</td>
                        </tr>
                        <tr>
                            <td class="text-xs text-left py-0.5 font-bold text-heading">
                                {{ $t('label.total') }}:
                            </td>
                ...
                <h4 v-if="order.token" class="py-2 capitalize text-xl font-bold text-center border-b border-dashed border-gray-400">
...
export default {
    ...
    mounted() {
        this.frontendSettingStore.fetch().then().catch();
    }
}
</script>
