...
export default {
    ...
    methods: {
        ...
        save: function () {
            try {
                const tempId = this.itemAttributeStore.temp.temp_id;
                this.loading.isActive = true;
                this.itemAttributeStore.save(this.props).then((res) => {
                    useModal().closeModal('modal');
                    this.loading.isActive = false;
                    alertService.successFlip(tempId === null ? 0 : 1, this.$t("menu.item_attributes"));
                    this.props.form = {
                        name: "",
                        status: statusEnum.ACTIVE
                    };
