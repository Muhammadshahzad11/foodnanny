            ...
            <div class="modal-body">
                <div v-if="message"
                    ...
                    <span class="absolute top-0 bottom-0 right-0 px-4 py-3 cursor-pointer" @click="close">
                        <i class="lab lab-line-circle-cross margin-top-5-px"></i>
                    </span>
                </div>
                <form @submit.prevent="save">
                    <div class="form-row">
                        <div class="form-col-12 sm:form-col-12">
                            ...
                            <small class="db-field-alert" v-if="errors.opening_time">{{ errors.opening_time[0] }}</small>
                        </div>
                        <div class="form-col-12 sm:form-col-12">
                            ...
                            <Datepicker hideInputIcon @update:modelValue="handleClosingTime" v-model="closing_time"
                                :input-class-name="errors.closing_time ? 'invalid' : ''" time-picker />
...
import SmTimeSlotModalCreateComponent from "../../components/buttons/SmTimeSlotModalCreateComponent.vue";
...
export default {
    ...
    methods: {
        ...
        save: function () {
            try {
                const tempId = this.timeSlotStore.temp.temp_id;
                this.loading.isActive = true;
                this.timeSlotStore.save(this.$props.props).then((res) => {
                    useModal().closeModal('modal');
