            ...
            <table class="db-table stripe" id="print">
                <thead class="db-table-head">
                    <tr class="db-table-head-tr">
                        ...
                        <th class="db-table-head-th hidden-print" v-if="permissionChecker('restaurants_show')">
                            {{ $t("label.action") }}
                        </th>
                    </tr>
                </thead>
                <tbody class="db-table-body" v-if="reviews.length > 0">
                    <tr class="db-table-body-tr" v-for="review in reviews" :key="review">
                        ...
                        <td class="db-table-body-td hidden-print" v-if="permissionChecker('restaurants_show')">
                            <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                                <button class="db-table-action view" @click="showModal(review)"  v-if="permissionChecker('restaurants_show')">
...
export default {
    ...
    mounted() {
        if(this.$route.params.id) {
            this.reviewProps.search.type          = modelTypeEnum.RESTAURANT;
            this.reviewProps.search.restaurant_id = this.$route.params.id;
        }
        this.list();
    },
    computed: {
    ...
    methods: {
        ...
        list: function (page = 1) {
            ...
            this.reviewProps.search.page = page;
            this.reviewStore.fetch(this.reviewProps.search).then((res) => {
