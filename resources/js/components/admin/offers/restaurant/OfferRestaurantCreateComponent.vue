<template>
    ...
    <div id="offerRestaurant" class="modal">
...
export default {
    name: "OfferRestaurantCreateComponent",
    ...
    methods: {
        add: function () {
            useModal().openModal('offerRestaurant');
        },
        reset: function () {
            useModal().closeModal('offerRestaurant');
        ...
        save: function () {
            try {
                ...
                this.loading.isActive = true;
                this.offerRestaurantStore.save(this.props).then((res) => {
                    useModal().closeModal('offerRestaurant');
                    this.loading.isActive = false;
                    alertService.successFlip(tempId === null ? 0 : 1, this.$t("label.restaurant"));
                    this.props.form = {restaurant_id: null};
                    this.errors = {};
                    this.message = null;
                }).catch((err) => {
                    this.loading.isActive = false;
