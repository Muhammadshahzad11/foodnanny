<template>
    ...
    <div class="col-12">
        <div class="flex flex-col items-start sm:flex-row sm:items-center gap-1.5 mb-6">
            ...
            <button type="button" @click="handleTab($event, 'image')" class="tab-button w-full flex items-center gap-2 px-4 h-10 rounded-lg bg-white hover:text-primary hover:bg-primary/10">
                <i class="lab lab-line-upload-image  lab-font-size-16"></i>
                <span class="capitalize text-sm">{{ $t("label.image") }}</span>
            </button>
            <button type="button" class="tab-button w-full flex items-center gap-2 px-4 h-10 rounded-lg bg-white hover:text-primary hover:bg-primary/10" @click="handleTab($event, 'restaurant')"><i class="lab lab-line-restaurants lab-font-size-16"></i>
                {{ $t('label.restaurants') }}
            </button>
            <button type="button" class="tab-button w-full flex items-center gap-2 px-4 h-10 rounded-lg bg-white hover:text-primary hover:bg-primary/10" @click="handleTab($event, 'translations')"><i class="lab lab-line-language lab-font-size-16"></i>
                    ...
                    <div class="row py-2">
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                ...
                                <span class="db-list-item-text w-full sm:w-1/2">{{ offer.title }}</span>
                            </div>
                        </div>
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                        ...
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                <span class="db-list-item-title w-full sm:w-1/2">{{ $t('label.date') }}</span>
                                <span class="db-list-item-text w-full sm:w-1/2">{{ offer.convert_date }}</span>
                            </div>
                        </div>
                        ...
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                ...
                                <span class="db-list-item-text w-full sm:w-1/2">{{ offer.convert_time }}</span>
                            </div>
                        </div>

                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                <span class="db-list-item-title w-full sm:w-1/2">{{ $t('label.location') }}</span>
                                <span class="db-list-item-text w-full sm:w-1/2">{{ offer.location }}</span>
                            </div>
                        </div>
                        ...
                        <div class="col-12 !py-1.5">
                            <div class="db-list-item p-0">
                                <span class="db-list-item-text mt-2 w-full">
                                    ...
                                    <span class="mt-2" v-html="offer.description"></span>
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="form-col-12 xl:form-col-6">
                            <fieldset class="p-4 mb-6 border border-[#DBDEE0]">
                                <legend class="py-1.5 px-4 text-base font-semibold capitalize border border-[#DBDEE0] text-primary"> {{ $t('label.thumbnail') }}  </legend>
                                <div class="row py-2">
                                    <form @submit.prevent="saveThumbnail" class="w-auto">
                                        ...
                                        <div class="flex gap-3 md:gap-4 p-4">
                                            <label for="thumbnail"
                                                ...
                                                <span class="hidden sm:inline-block">{{ $t("button.upload_new_thumbnail") }}</span>
                                                <input v-if="uploadThumbnailButton" @change="changePreviewThumbnail" ref="thumbnailProperty" accept="image/png, image/jpeg, image/jpg" type="file" id="thumbnail" class="absolute top-0 left-0 w-full h-full -z-10 opacity-0" />
                                            </label>
                                            <button v-if="saveThumbnailButton" type="submit" class="db-btn h-[38px] shadow-[0px_6px_10px_rgba(26,_183,_89,_0.24)] text-white bg-[#1AB759]">
                                                <i class="lab lab-line-circle-check"></i>
                                                <span class="hidden sm:inline-block">{{ $t("button.save") }}</span>
                                            </button>
                                            <button v-if="resetThumbnailButton" @click="resetPreviewThumbnail" type="button" class="db-btn-outline h-[38px] shadow-[0px_6px_10px_rgba(251,_78,_78,_0.24)] !text-[#FB4E4E] !bg-white !border-[#FB4E4E]">
                                ...
                                <div class="col-12 sm:col-5">
                                    <img class="db-image" alt="offer-thumbnail" :src="previewThumbnail" />
                                </div>
                            </fieldset>
                        </div>
                        ...
                        <div class="form-col-12 xl:form-col-6">
                            <fieldset class="p-4 mb-6 border border-[#DBDEE0]">
                                <legend class="py-1.5 px-4 text-base font-semibold capitalize border border-[#DBDEE0] text-primary"> {{ $t('label.cover') }} </legend>
                                <div class="row py-2">
                                    <form @submit.prevent="saveCover" class="w-auto">
                                        <p class="mt-2 px-4">{{ $t('label.size') }}: (1120px,270px)</p>
                                        <div class="flex gap-3 md:gap-4 p-4">
                                            <label for="photo"
                                                class="db-btn relative cursor-pointer h-[38px] shadow-[0px_6px_10px_rgb(var(--primary)/0.24)] bg-primary text-white">
                                                <i class="lab lab-line-upload-image"></i>
                                                <span class="hidden sm:inline-block">{{ $t("button.upload_new_cover") }}</span>
                                                <input v-if="uploadButton" @change="changePreviewCover" ref="coverProperty" accept="image/png, image/jpeg, image/jpg" type="file" id="photo" class="absolute top-0 left-0 w-full h-full -z-10 opacity-0" />
                                            </label>
                                ...
                                <div class="col-12 sm:col-5">
                                    <img class="db-image" alt="offer-cover" :src="previewCover" />
                                </div>
                            </fieldset>
                        </div>
                    </div>
        ...
        <div class="tab-content db-card" id="restaurant">
            <OfferRestaurantListComponent :offer="parseInt($route.params.id)" />
        </div>
...
import OfferRestaurantListComponent from "./restaurant/OfferRestaurantListComponent.vue";
import OfferTranslationComponent from "./OfferTranslationComponent.vue";

export default {
    name: "OfferShowComponent",
    components: {
        LoadingComponent,
        OfferRestaurantListComponent,
        OfferTranslationComponent
    },
    setup() {
    ...
    mounted() {
        ...
        this.offerStore.view(this.$route.params.id).then(res => {
        ...
        saveThumbnail: function () {
            if (this.$refs.thumbnailProperty.files[0]) {
                try {
                    ...
                    this.offerStore.changeThumbnail({id: this.$route.params.id, form: formData}).then((res) => {
        ...
        saveCover: function () {
            if (this.$refs.coverProperty.files[0]) {
                try {
                    ...
                    this.offerStore.changeCover({ id: this.$route.params.id, form: formData}).then((res) => {
