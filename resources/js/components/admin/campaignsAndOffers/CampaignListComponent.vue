        ...
        <div class="table-filter-div" id="campaign-filter">
            <form class="p-4 sm:p-5 mb-5" @submit.prevent="search">
                <div class="row">
                    ...
                    <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                        <label for="searchStartDate" class="db-field-title after:hidden"> {{ $t('label.time') }} </label>
            ...
            <table class="db-table stripe" id="print">
                ...
                <tr class="db-table-body-tr" v-for="campaign in campaigns" :key="campaign">
                    <td class="db-table-body-td">
                        <div v-if="campaign.title.length < 40">{{ campaign.title }}</div>
                        <div v-else>{{ campaign.title.substring(0, 40) + ".." }}</div>
                    </td>
                    ...
                    <td class="db-table-body-td hidden-print" v-if="permissionChecker('campaigns-and-offers')">
                        <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                            <SmIconViewComponent :link="'admin.campaignsAndOffers.showCampaign'"
                                                 :id="campaign.id"/>
                            <SmIconIconApplyComponent @click="apply(campaign.id)"
                                                      v-if="campaign.apply === enums.askEnum.NO"/>
                            <SmIconLeaveComponent @click="leave(campaign.id)" v-else/>
                        </div>
                    </td>
                </tr>
                </tbody>
...
export default {
    ...
    computed: {
        ...
        campaigns: function () {
            return this.campaignAndOfferStore.campaignLists;
        },
        pagination: function () {
            return this.campaignAndOfferStore.campaignPagination;
        },
        paginationPage: function () {
            return this.campaignAndOfferStore.campaignPage;
        }
    },
    methods: {
        permissionChecker(e) {
        ...
        list: function (page = 1) {
            ...
            this.campaignAndOfferStore.fetchCampaignLists(this.props.search).then((res) => {
        ...
        apply: function (id) {
            return new VueSimpleAlert.confirm(
                ...
                try {
                    this.loading.isActive = true;
                    this.campaignAndOfferStore.fetchApplyCampaign({id: id, search: this.props.search}).then((res) => {
                        this.loading.isActive = false;
                        alertService.successInfo(null, this.$t("message.apply_campaign"));
                    }).catch((err) => {
                        this.loading.isActive = false;
        ...
        leave: function (id) {
            return new VueSimpleAlert.confirm(
                ...
                try {
                    this.loading.isActive = true;
                    this.campaignAndOfferStore.leaveCampaign({id: id, search: this.props.search}).then((res) => {
                        this.loading.isActive = false;
                        alertService.successInfo(null, this.$t("message.leave_campaign"));
                    }).catch((err) => {
                        this.loading.isActive = false;
                        alertService.error(err.response.data.message);
                    });
        ...
        xls: function () {
            this.loading.isActive = true;
            this.campaignAndOfferStore.exportCampaign(this.props.search).then((res) => {
                this.loading.isActive = false;
