<template>
    ...
    <div class="col-12" v-if="Object.entries(deliveryBoy).length > 0">
        ...
        <div class="flex flex-col items-start sm:flex-row sm:flex-wrap sm:items-center gap-1.5 mb-6">
            <button type="button" @click="handleTab($event, 'profile')"
                class="tab-button tab-active w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-line-user"></i>
                <span class="capitalize text-sm">{{ $t('button.profile') }}</span>
            </button>
            <button type="button" @click="handleTab($event, 'security')"
                class="tab-button w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-line-unlock"></i>
                <span class="capitalize text-sm">{{ $t('button.security') }}</span>
            </button>
            <button type="button" @click="handleTab($event, 'address')"
                class="tab-button w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-line-location"></i>
                <span class="capitalize text-sm">{{ $t('button.address') }}</span>
            </button>
            <button type="button" @click="handleTab($event, 'orders')"
                class="tab-button w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-line-reserve"></i>
                <span class="capitalize text-sm">{{ $t('button.my_orders') }}</span>
            </button>
            <button type="button" @click="handleTab($event, 'deliveredOrders')"
                class="tab-button w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-line-delivered-order"></i>
                <span class="capitalize text-sm">{{ $t('button.delivered_orders') }}</span>
            </button>
            <button type="button" @click="handleTab($event, 'deliveryBoyCollection')"
                class="tab-button w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-line-collection"></i>
                <span class="capitalize text-sm">{{ $t('button.collection') }}</span>
            </button>
            <button type="button" @click="handleTab($event, 'deliveryBoyReview')"
                class="tab-button w-full justify-start sm:w-fit inline-flex items-center sm:justify-center gap-2 h-[38px] py-2 px-4 rounded-md text-[#6E7191] stroke-[#6E7191]">
                <i class="lab lab-line-review"></i>
                <span class="capitalize text-sm">{{ $t('button.reviews') }}</span>
            </button>
        </div>
                    ...
                    <div class="row py-2">
                        ...
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                ...
                                <span class="db-list-item-text w-full sm:w-1/2">{{ deliveryBoy.country_code + '' + deliveryBoy.phone }}</span>
                            </div>
                        </div>
                        <div class="col-12 sm:col-6 !py-1.5">
                            <div class="db-list-item p-0">
                                ...
                                <span class="db-list-item-text w-full sm:w-1/2">
                                    <span :class="statusClass(deliveryBoy.status)">{{ enums.statusEnumArray[deliveryBoy.status] }}</span>
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <form @submit.prevent="changePassword">
                        <div class="form-row">
                            <div class="form-col-12 sm:form-col-6">
                                <label for="password" class="db-field-title required">{{ $t("label.new_password") }}</label>
                                <input v-model="password.props.form.password" v-bind:class="password.errors.password ? 'invalid' : ''" type="password" id="password" class="db-field-control" autocomplete="off" />
                                <small class="db-field-alert" v-if="password.errors.password">{{ password.errors.password[0] }}</small>
                            </div>

                            <div class="form-col-12 sm:form-col-6">
                                <label for="password" class="db-field-title required"> {{
                                    $t("label.confirm_new_password") }} </label>
                                ...
                                <small class="db-field-alert" v-if="password.errors.password_confirmation">{{
                                    password.errors.password_confirmation[0] }}</small>
                            </div>
        ...
        <div id="address" class="tab-content">
            <DeliveryBoyAddressListComponent :props="$route.params.id" />
        </div>
            ...
            <div class="db-card">
                ...
                <div class="db-card-body">
                    <div class="row">
                            ...
                            <div class="w-full rounded-lg py-2 px-3 flex items-center gap-5 border border-[#EFF0F6]">
                                ...
                                <div class="w-full">
                                    <div class="flex items-center gap-4 mb-1">
                                        ...
                                        <span class="py-0.5 px-2 rounded-full text-[10px] font-rubik leading-4 capitalize text-heading bg-[#BDEFFF]" :class="orderStatusClass(order.status)">
                                            {{ order.status_name }}
                                        </span>
                                    </div>
                                    <p class="text-xs font-light font-rubik mb-0.5">
                                        {{ order.order_items }} {{ $t("label.items") }}
                                    </p>
                                    <p class="text-xs font-light font-rubik mb-1">{{ order.order_datetime }}</p>
                                    <div class="flex items-center justify-between">
                                        <p class="text-sm leading-6 font-rubik capitalize text-heading">{{ $t("label.total") }}:
                                            <span class="font-medium">{{ order.total_currency_price }}</span>
                                        </p>
                                        <router-link
                                            :to="{ name: 'admin.deliveryBoys.order.details', params: { id: $route.params.id, orderId: order.id } }"
                        ...
                        <div class="col-12 p-4 d-flex justify-center" v-else>
        ...
        <div id="deliveryBoyCollection" class="tab-content">
            <CollectionListComponent />
        </div>
        <div id="deliveryBoyReview" class="tab-content">
            <ReviewListComponent />
        </div>
    </div>
</template>
...
import DeliveryBoyAddressListComponent from "./address/DeliveryBoyAddressListComponent.vue";
import DeliveredOrderList from "./deliveredOrder/DeliveredOrderList.vue";
...
import CollectionListComponent from "./collection/CollectionListComponent.vue";
import ReviewListComponent from "./review/ReviewListComponent.vue";

export default {
    name: "DeliveryBoyShowComponent",
    components: {
        DeliveryBoyAddressListComponent,
        DeliveredOrderList,
        LoadingComponent,
        PaginationSMBox,
        PaginationBox,
    ...
    computed: {
        ...
        deliveryBoy: function () {
            return this.deliveryBoyStore.show;
        },
        myOrders: function () {
            return this.deliveryBoyStore.myOrders;
        },
        orderPagination: function () {
            return this.deliveryBoyStore.orderPagination;
        },
        orderPage: function () {
            return this.deliveryBoyStore.orderPage;
        }
    },
    mounted() {
        this.loading.isActive = true;
        this.deliveryBoyStore.view(this.$route.params.id).then((res) => {
    ...
    methods: {
        ...
        orderLists: function (page = 1) {
            ...
            this.deliveryBoyStore.callMyOrders({
        ...
        saveImage: function () {
            if (this.$refs.imageProperty.files[0]) {
                try {
                    ...
                    this.deliveryBoyStore.changeImage({
                        id: this.$route.params.id,
                        form: formData
                    }).then((res) => {
        ...
        changePassword: function () {
            try {
                const tempId = this.deliveryBoyStore.temp.temp_id;
                this.loading.isActive = true;
                this.deliveryBoyStore.changePassword({
