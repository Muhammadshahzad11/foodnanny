                    ...
                    <div class="w-full rounded-lg py-2 px-3 flex items-center gap-5 border border-[#EFF0F6]">
                        ...
                        <div class="w-full">
                            <div class="flex items-center gap-4 mb-1">
                                <p class="text-sm leading-6 font-rubik">{{ $t('label.order_id') }}: <span class="text-heading">#{{order.order_serial_no}}</span>
                                </p>
                                <span class="py-0.5 px-2 rounded-full text-[10px] font-rubik leading-4 capitalize text-heading bg-[#BDEFFF]"
                                    :class="orderStatusClass(order.status)"> {{ order.status_name }}</span>
                            </div>
                            <p class="text-xs font-light font-rubik mb-0.5">{{ order.order_items }} {{$t("label.items") }}</p>
                            <p class="text-xs font-light font-rubik mb-1">{{ order.order_datetime }}</p>
                            <div class="flex items-center justify-between">
                                <p class="text-sm leading-6 font-rubik capitalize text-heading">{{$t("label.total") }}: <span class="font-medium">{{ order.total_currency_price }}</span></p>
                                <router-link
                                    :to="{ name: 'admin.deliveryBoys.deliveredOrder.details', params: { id: props, orderId: order.id } }"
...
export default {
    ...
    computed: {
        ...
        deliveredOrders: function () {
            return this.deliveryBoyOrderStore.deliveredOrders;
        },
        deliveredOrderPagination: function () {
            return this.deliveryBoyOrderStore.deliveredOrderPagination;
        },
        deliveredOrderPage: function () {
            return this.deliveryBoyOrderStore.deliveredOrderPage;
        }
    },
    methods: {
        ...
        deliveredOrderLists: function (page = 1) {
            ...
            this.deliveryBoyOrderStore.callDeliveredOrders({
                id: this.props,
                search: this.search
            }).then(res => {
