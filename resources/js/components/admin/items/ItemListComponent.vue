            ...
            <div class="table-filter-div" id="item-filter">
                <form class="p-4 sm:p-5 mb-5" @submit.prevent="search">
                    <div class="row">
                        ...
                        <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                            <label for="price" class="db-field-title after:hidden">{{ $t("label.price") }}</label>
                        ...
                        <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                            <label for="searchItemType" class="db-field-title after:hidden">{{ $t("label.item_type") }}</label>
                        ...
                        <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                            <label for="searchIsHalal" class="db-field-title after:hidden">{{ $t("label.is_it_halal") }}</label>
                            <vue-select class="db-field-control f-b-custom-select" id="searchIsHalal"
                                v-model="props.search.is_halal" :options="[
...
export default {
    ...
    mounted() {
        ...
        this.itemCategoryStore.fetch(this.categoryProps.search).then(res => {
            this.loading.isActive = false;
        }).catch((err) => {
            this.loading.isActive = false;
        });
        this.taxStore.fetch(this.taxProps.search).then(res => {
            this.loading.isActive = false;
        }).catch((err) => {
            this.loading.isActive = false;
        });
    ...
    computed: {
        ...
        pagination: function () {
            return this.itemStore.pagination;
        },
        paginationPage: function () {
            return this.itemStore.page;
        },
    ...
    methods: {
        ...
        clear: function () {
            ...
            this.props.search.is_halal = null;
        ...
        edit: function (item) {
            ...
            this.loading.isActive = true;
            this.itemStore.edit(item.id);
            this.loading.isActive = false;
            this.props.errors = {};
            this.props.startTime = this.parseTime(item.available_time_start);
            this.props.endTime = this.parseTime(item.available_time_end);
            this.props.form = {
                name: item.name,
                ...
                status: item.status,
                is_halal: item.halal,
                available_time_start: item.available_time_start,
                available_time_end: item.available_time_end,
                discount_type: item.discount_type == "" ? null : item.discount_type,
                discount: item.flat_discount,
                maximum_purchase_quantity: item.maximum_purchase_quantity
            };
        },
        parseTime(time) {
            if (time === "" || time === null) {
        ...
        destroy: function (id) {
            return new VueSimpleAlert.confirm(
                ...
                try {
                    this.loading.isActive = true;
                    this.itemStore.destroy({ id: id, search: this.props.search }).then((res) => {
                        this.loading.isActive = false;
        ...
        xls: function () {
            this.loading.isActive = true;
            this.itemStore.export(this.props.search).then((res) => {
                this.loading.isActive = false;
