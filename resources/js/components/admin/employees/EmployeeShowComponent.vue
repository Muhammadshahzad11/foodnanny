<template>
    ...
    <div class="col-12" v-if="Object.entries(employee).length > 0">
                ...
                <div class="db-card-body">
                    <form @submit.prevent="changePassword">
                        <div class="form-row">
                            ...
                            <div class="form-col-12 sm:form-col-6">
                                <label for="password" class="db-field-title required">{{ $t("label.confirm_new_password") }} </label>
        ...
        <div id="address" class="tab-content">
            <EmployeeAddressListComponent :props="$route.params.id"/>
        </div>
                                ...
                                <div class="w-full">
                                    <div class="flex items-center gap-4 mb-1">
                                        <p class="text-sm leading-6 font-rubik">{{ $t('label.order_id') }}: <span
                                            class="text-heading">#{{ order.order_serial_no }}</span></p>
                                        <span
                                    ...
                                    <div class="flex items-center justify-between">
                                        <p class="text-sm leading-6 font-rubik capitalize text-heading">
                                            {{ $t("label.total") }}: <span
                                            class="font-medium">{{ order.total_currency_price }}</span>
                                        </p>
                                        <router-link
...
import EmployeeAddressListComponent from "./address/EmployeeAddressListComponent.vue";
...
export default {
    ...
    computed: {
        ...
        employee: function () {
            return this.employeeStore.show;
        },
        myOrders: function () {
            return this.employeeStore.myOrders;
        },
        orderPagination: function () {
            return this.employeeStore.orderPagination;
        },
        orderPage: function () {
            return this.employeeStore.orderPage;
        },
    },
    mounted() {
        this.loading.isActive = true;
        this.employeeStore.view(this.$route.params.id).then((res) => {
    ...
    methods: {
        ...
        orderLists: function (page = 1) {
            ...
            this.employeeStore.callMyOrders({
        ...
        saveImage: function () {
            if (this.$refs.imageProperty.files[0]) {
                try {
                    ...
                    this.employeeStore.changeImage({id: this.$route.params.id,form: formData}).then((res) => {
                    ...
                    this.loading.isActive = false;
                    alertService.error(err.response.data.errors.image);
                }
            }
        },
        changePassword: function () {
            try {
                const tempId          = this.employeeStore.temp.temp_id;
                this.loading.isActive = true;
                this.employeeStore.changePassword({
