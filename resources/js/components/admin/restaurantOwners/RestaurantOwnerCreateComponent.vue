    ...
    <div id="sidebar" @click="closeBackdrop" class="fixed inset-0 z-50 bg-black/50 duration-500 transition-all invisible opacity-0">
        <div class="w-full max-w-xl h-dvh overflow-x-hidden thin-scrolling bg-white ms-auto ltr:translate-x-full rtl:-translate-x-full">
            <div class="flex items-center justify-between p-4 border-b border-slate-100">
                <h3 class="drawer-title">{{ $t("menu.restaurant_owners") }}</h3>
                ...
                <form @submit.prevent="save">
                    <div class="form-row">
                        ...
                        <div class="form-col-12 sm:form-col-6">
                            ...
                            <vue-select class="db-field-control f-b-custom-select" id="restaurant_id" v-bind:class="errors.restaurant_id ? 'invalid' : ''"
                                v-model="props.form.restaurant_id" :options="restaurants" label-by="name" value-by="id"
...
import { useRestaurantOwnerStore } from "../../../stores/restaurantOwner.js";
...
export default {
    name: "RestaurantOwnerCreateComponent",
    ...
    data() {
        return {
            ...
            addButton: {
                title: this.$t("button.add_restaurant_owner")
            },
            enums: {
    ...
    async mounted() {
        ...
        await this.restaurantStore.fetchAllRestaurant().then(res => {
            this.loading.isActive = false;
        }).catch((err) => {
            this.loading.isActive = false;
        });
    ...
    methods: {
        ...
        addReset: function () {
            this.restaurantOwnerStore.reset();
            this.errors = {};
        ...
        save: function () {
            try {
                ...
                this.loading.isActive = true;
                this.restaurantOwnerStore.save(this.props).then((res) => {
                        useCanvas().closeCanvas('sidebar');
                        this.loading.isActive = false;
                        alertService.successFlip(tempId === null ? 0 : 1, this.$t("menu.restaurant_owners"));
                        this.props.form = {
                            name                 : "",
                            email                : "",
                            phone                : "",
