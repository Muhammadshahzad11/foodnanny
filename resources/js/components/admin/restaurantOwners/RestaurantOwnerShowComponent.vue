<template>
    ...
    <div class="col-12" v-if="Object.entries(restaurantOwner).length > 0">
                    ...
                    <form @submit.prevent="changePassword">
                        <div class="form-row">
                            <div class="form-col-12 sm:form-col-6">
                                <label for="password" class="db-field-title required">{{ $t("label.new_password")
                                    }}</label>
        ...
        <div id="address" class="tab-content">
            <RestaurantOwnerAddressListComponent :props="$route.params.id" />
        </div>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <p class="text-sm leading-6 font-rubik capitalize text-heading">
                                            {{ $t("label.total") }}: <span class="font-medium">{{
                                                order.total_currency_price }}</span>
                                        </p>
                                        <router-link
                                            :to="{ name: 'admin.restaurantOwners.order.details', params: { id: $route.params.id, orderId: order.id } }"
...
import RestaurantOwnerAddressListComponent from "./address/RestaurantOwnerAddressListComponent.vue";
...
export default {
    name: "RestaurantOwnerShowComponent",
    components: {
        RestaurantOwnerAddressListComponent,
        LoadingComponent,
        PaginationSMBox,
        PaginationBox,
    ...
    computed: {
        ...
        restaurantOwner: function () {
            return this.restaurantOwnerStore.show;
        },
        myOrders: function () {
            return this.restaurantOwnerStore.myOrders;
        },
        orderPagination: function () {
            return this.restaurantOwnerStore.orderPagination;
        },
        orderPage: function () {
            return this.restaurantOwnerStore.orderPage;
        }
    },
    mounted() {
        this.loading.isActive = true;
        this.restaurantOwnerStore.view(this.$route.params.id).then((res) => {
    ...
    methods: {
        ...
        orderLists: function (page = 1) {
            ...
            this.restaurantOwnerStore.callMyOrders({
        ...
        saveImage: function () {
            if (this.$refs.imageProperty.files[0]) {
                try {
                    ...
                    this.restaurantOwnerStore.changeImage({ id: this.$route.params.id, form: formData }).then((res) => {
        ...
        changePassword: function () {
            try {
                const tempId = this.restaurantOwnerStore.temp.temp_id;
                this.loading.isActive = true;
                this.restaurantOwnerStore.changePassword({ form: this.password.props.form, id: this.$route.params.id }).then((res) => {
                    this.loading.isActive = false;
