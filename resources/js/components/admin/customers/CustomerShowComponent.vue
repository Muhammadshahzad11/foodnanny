<template>
    ...
    <div class="col-12" v-if="Object.entries(customer).length > 0">
        ...
        <div id="address" class="tab-content">
            <CustomerAddressListComponent :props="$route.params.id" />
        </div>
                                ...
                                <div class="w-full">
                                    <div class="flex items-center gap-4 mb-1">
                                        <p class="text-sm leading-6 font-rubik">{{ $t('label.order_id') }}: <span
                                                class="text-heading">#{{order.order_serial_no}}</span>
                                        </p>
                                        <span
                                    ...
                                    <div class="flex items-center justify-between">
                                        <p class="text-sm leading-6 font-rubik capitalize text-heading">
                                            {{ $t("label.total") }}: <span class="font-medium">{{order.total_currency_price}}</span></p>
                                        <router-link
...
import CustomerAddressListComponent from "./address/CustomerAddressListComponent.vue";
...
export default {
    ...
    computed: {
        ...
        customer: function () {
            return this.customerStore.show;
        },
        myOrders: function () {
            return this.customerStore.myOrders;
        },
        orderPagination: function () {
            return this.customerStore.orderPagination;
        },
        orderPage: function () {
            return this.customerStore.orderPage;
        },
    },
    mounted() {
        this.loading.isActive = true;
        this.customerStore.view(this.$route.params.id).then((res) => {
    ...
    methods: {
        ...
        orderLists: function (page = 1) {
            ...
            this.customerStore.callMyOrders({
        ...
        saveImage: function () {
            if (this.$refs.imageProperty.files[0]) {
                try {
                    ...
                    this.customerStore.changeImage({
                        id: this.$route.params.id,
                        form: formData,
                    })
                        .then((res) => {
        ...
        changePassword: function () {
            try {
                ...
                this.loading.isActive = true;
                this.customerStore.changePassword({
