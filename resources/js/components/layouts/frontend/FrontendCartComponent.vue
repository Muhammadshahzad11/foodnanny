<template>
    <aside id="cart-canvas" @click="closeBackdrop"
        ...
        <div
            ...
            <div v-if="(carts.length > 0 && orderType === null && Object.keys(restaurant).length > 0)"
                 class="h-[calc(100dvh-274px)] w-full py-10 flex flex-col items-center justify-center text-center">
            ...
            <ul v-if="carts.length > 0" class="px-4 h-[calc(100dvh-274px)] thin-scrolling">
                <li v-for="(cart, index) in carts" :key="index" class="py-4 border-t border-gray-100">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex items-center w-16 h-6 gap-1 self-end flex-shrink-0">
                            <button @click.prevent="quantityDecrement(index)"
                                    :class="cart.quantity === 1 ? 'lab-line-trash text-danger' : 'lab-line-minus-circle'"
                                    class="font-medium hover:text-primary"></button>
            ...
            <div v-if="carts.length > 0" class="p-4 h-[140px]">
                ...
                <router-link @click.prevent="closeCanvas('cart-canvas')" :to="{ name: 'frontend.checkout' }"
                             :class="restaurant.order_setup.minimum_order_limit > 0 && restaurant.order_setup.minimum_order_limit > subtotal ? 'bg-primary/50 pointer-events-disable text-sm' : ''"
                             class="w-full py-3 px-3 block font-medium text-white bg-primary rounded-3xl text-center">
                    {{ $t('button.proceed_checkout') }}
                    <strong
                        v-if="restaurant.order_setup.minimum_order_limit > 0 && restaurant.order_setup.minimum_order_limit > subtotal"
                        class="text-red-400">
