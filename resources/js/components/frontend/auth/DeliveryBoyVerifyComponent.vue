    ...
    <section class="pb-16">
        <div class="w-full max-w-[360px] mx-auto mt-8 mb-12 p-5 rounded-2xl bg-white shadow-xs">
            ...
            <form @submit.prevent="save">
                <label for="token" class="field-title !normal-case">{{ $t('label.enter_the_code_sent_to') }}
                    <span class="font-medium">
                        {{ props.form.code + '' + props.form.phone }}
                    </span>
                </label>
                <input id="token" :class="errors !== '' ? 'invalid' : ''" type="text" v-model="props.form.token"
                       class="field-control">
...
export default {
    name: "DeliveryBoyVerifyComponent",
    ...
    methods: {
        ...
        resendCode: function () {
            try {
                ...
                this.frontendDeliveryBoySignupStore.callPhone({
                    code: this.frontendDeliveryBoySignupStore.code,
                    phone: this.frontendDeliveryBoySignupStore.phone
                }).then(res => {
        ...
        save: function () {
            try {
                this.loading.isActive = true;
                this.frontendDeliveryBoySignupStore.callVerify(this.props.form).then(res => {
                    this.loading.isActive = false;
                    alertService.success(res.data.message);
                    this.props.form = {
                        code: "",
                    ...
                    this.errors     = '';
                    this.$router.push({name: "auth.signupDeliveryBoyRegister"});
                }).catch((err) => {
                    this.loading.isActive = false;
                    this.errors           = err.response.data.message;
                });
