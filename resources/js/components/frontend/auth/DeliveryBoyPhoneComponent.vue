                ...
                <div class="mb-6">
                    ...
                    <div class="relative group">
                        <div :class="[isOpen ? 'rounded-t-lg' : 'rounded-lg', errors.phone ? 'invalid' : '']" class="flex w-full h-12 border border-[#d9dbe9] group-hover:border-primary/20 transition-all duration-300 group-focus-within:border-primary/20">
                            ...
                            <input id="phone" v-model="props.form.phone" v-on:keypress="phoneNumber($event)" type="text" class="pl-2 pr-3 w-full overflow-hidden h-full placeholder:text-sm">
                        </div>
...
export default {
    name: "DeliveryBoyPhoneComponent",
    ...
    methods: {
        ...
        save: function () {
            try {
                this.loading.isActive = true;
                this.frontendDeliveryBoySignupStore.callPhone(this.props.form).then(res => {
                    this.loading.isActive = false;
                    this.props.form       = {
                        phone: "",
                        code: "",
                    ...
                    if (this.demo.toLowerCase() === "true") {
                        ...
                        if (this.setting.site_phone_verification === this.enums.askEnum.NO) {
                            ...
                            this.$router.push({name: "auth.signupDeliveryBoyVerify"});
                        }
                    }
                }).catch((err) => {
                    this.loading.isActive = false;
