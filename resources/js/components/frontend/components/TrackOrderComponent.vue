<template>
    ...
    <div v-if="logged && activeOrders.length > 0" class="xst:rounded-t-2xl xst:fixed xst:bottom-12 xst:w-full xst:z-30 bg-[#FFF8F2] sm:bg-[#FF8C39]">
        <div class="relative w-full lg:w-[930px] mx-auto flex items-center gap-4 py-2.5 sm:px-10 px-4 xst:pb-16">
            <i class="lab-line-routing text-2xl text-primary sm:text-white"></i>
            <div class="flex-auto">
                <h3 class="text-sm mb-1.5 font-medium first-letter:capitalize text-primary sm:text-white">
                    {{ $t('message.order_process_stage') }}
                </h3>
                ...
                <p v-if="Object.keys(currentOrder).length > 0 && parseInt(currentOrder.status) === parseInt(enums.orderStatusEnum.PREPARING) || parseInt(currentOrder.status) === parseInt(enums.orderStatusEnum.PREPARED)"
                   class="text-xs text-primary sm:text-white">{{ $t('message.order_preparing') }}</p>
            ...
            <img class="xst:absolute xst:bottom-5 xst:left-1/2 xst:-translate-x-1/2 sm:w-48 md:w-auto lg:mx-14" :src="settings.image_order_track" alt="bg">
            <router-link v-if="Object.keys(currentOrder).length > 0" :to="{ name: 'frontend.myOrder.details', params: { id: currentOrder.id } }" class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-primary sm:bg-white">
                <i class="lab-line-long-arrow-right text-xl text-white sm:text-primary"></i>
            </router-link>
        </div>
    </div>
</template>
    ...
    mounted() {
        try {
            if (this.authStore.status) {
                ...
                this.frontendOrderStore.fetchActiveOrder(this.activeProps).then(res => {
