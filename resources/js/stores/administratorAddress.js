...
export const useAdministratorAddressStore = defineStore('administratorAddress', {
    state: () => ({
        lists: [],
        page: {},
        pagination: [],
    ...
    actions: {
        ...
        save: function (payload) {
            return new Promise((resolve, reject) => {
                ...
                if (this.temp.isEditing) {
                    method = axios.put;
                    url = `/admin/administrator/address/${payload.id}/${this.temp.temp_id}`;
                }
        ...
        destroy: function (payload) {
            return new Promise((resolve, reject) => {
                axios.delete(`admin/administrator/address/${payload.id}/${payload.addressId}`).then((res) => {
                    this.fetch({ id: payload.id, search: payload.search }).then().catch();
                    resolve(res);
                }).catch((err) => {
                    reject(err);
        ...
        view: function (payload) {
            return new Promise((resolve, reject) => {
                axios.get(`admin/administrator/address/show/${payload.id}/${payload.addressId}`).then((res) => {
                    this.temp.temp_id = null;
                    this.temp.isEditing = false;
                    resolve(res);
                }).catch((err) => {
