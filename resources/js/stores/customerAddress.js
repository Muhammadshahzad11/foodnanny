...
export const useCustomerAddressStore = defineStore('customerAddress', {
    ...
    actions: {
        ...
        save: function (payload) {
            return new Promise((resolve, reject) => {
                ...
                if (this.temp.isEditing) {
                    method = axios.put;
                    url = `/admin/customer/address/${payload.id}/${this.temp.temp_id}`;
                }
        ...
        destroy: function (payload) {
            return new Promise((resolve, reject) => {
                axios.delete(`admin/customer/address/${payload.id}/${payload.addressId}`).then((res) => {
                    this.fetch({ id: payload.id, search: payload.search }).then().catch();
                    resolve(res);
                }).catch((err) => {
                    reject(err);
        ...
        view: function (payload) {
            return new Promise((resolve, reject) => {
                axios.get(`admin/customer/address/show/${payload.id}/${payload.addressId}`).then((res) => {
