...
<div class="py-14 px-4 w-full max-w-3xl mx-auto">
    ...
    <div id="loading-show" class="mx-auto w-80 {{ session()->has('error') ? 'hidden' : '' }}">
        <img class="w-full" src="{{ asset('images/required/payment-loading.gif') }}" alt="loading">
    </div>
    ...
    <form id="paymentForm" class="w-full" method="POST"
          action="{{ route('payment.store', ['frontendOrder' => $frontendOrder]) }}">
        @csrf
        ...
        <div id="backBtn" class="py-5 px-4 w-full max-w-3xl mx-auto flex flex-col items-center justify-center {{ !session()->has('error') ? 'hidden' : '' }}">
            @if($frontendOrder->order_type == \App\Enums\OrderType::DELIVERY || $frontendOrder->order_type == \App\Enums\OrderType::TAKEAWAY)
