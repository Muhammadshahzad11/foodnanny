...
@section('container')
    <ul class="installer-track">
        ...
        <li onclick="handleLinkForInstaller('{{ route('installer.site') }}')" class="done">
            <i class="lab-fill-site"></i>
        </li>
        <li class="active"><i class="lab-fill-database"></i></li>
    ...
    <form method="post" action="{{ route('installer.databaseStore') }}">
        ...
        <div class="mb-4">
            ...
            @if ($errors->has('database_username'))
                <small class="block mt-2 text-sm font-medium text-[#E93C3C]">{{ $errors->first('database_username') }}</small>
            @endif
        </div>
        ...
        <div class="mb-8">
            ...
            <input name="database_password" value="{{ old('database_password') }}" type="text" class="w-full h-12 rounded-lg px-4 border border-[#D9DBE9]">
            @if ($errors->has('database_password'))
                <small class="block mt-2 text-sm font-medium text-[#E93C3C]">{{ $errors->first('database_password') }}</small>
            @endif
        </div>
